...
# Diretorio:
# setwd(r"(D:\Estudos\Atuária\2025.1\População, Sociedade e Ambiente\Unidade III\Atividade Final - VII – Integridade Física e Liberdade de Orientação Sexual)")
...
#   ppt <- ph_with(ppt, dml(ggobj = last_plot()), 
#     location = ph_location(width = LARGURA, height = 6.75, left = 0, top = 0))
#   
...
# Dados2 <- Dados %>% filter(!Orientação_sexual_da_vítima
...
# #
# #   ggplot(., aes(x = Mes, y = Total, group = `Orientação Sexual`,
# #                    fill = `Orientação Sexual`)) +
# #
# #     geom_bar(stat = "identity") + scale_fill_paletteer_d("rcartocolor::Prism") +
# #
...
GDenunciaCorRaca <- function(.){
  ...
  ggplot(., aes(x = `Orientação Sexual`, y = Total, fill = `Orientação Sexual`)) +
...
DenunciaFaixaEtariaVitima <- Dados4 %>%
  ...
  filter(Faixa_etária_da_vítima2 != "Não identificado") %>%
...
DenunciaFaixaEtariaSuspeito <- Dados4 %>%
  ...
  filter(Faixa_etária_do_suspeito2 != "Não identificado") %>%
...
GDenunciaFaixaEtaria <- function(., PercentualFaixaEtaria){
  ...
  ggplot(., aes(x = `Faixa Etária`, y = Total, fill = `Orientação Sexual`)) +
...
DenunciaClassiSuspeito <- Dados4 %>%
  ...
  filter(Relação_vítima_suspeito2 != "Própria vítima") %>%
...
DenunciasEstadoFull <- left_join(GeoEstado, DenunciasEstado,
